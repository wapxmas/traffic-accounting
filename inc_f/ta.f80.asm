;;func_ls_by_port_ip_each;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_port_ip_each_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_PORT_EIP
	mov	dword[current.nfunc],ecx
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_ls_by_port_ip_each
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
	...
	;;;
	ret
do_ls_by_port_ip_each:
	test	byte[.first],1
	jnz	.next2
	setz	byte[.first]
	mov	edx,on_timer_ls_by_port_ip_each
	call	add_to_timer_f	
    .next2:
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_PORT_EIP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_PORT_EIP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
	...
	jz	.tsrc
	setz	byte[inc_by_port_ip_each_ls.type]
	mov	eax,dword[packet.da]
	bswap	eax
	call	inc_by_port_ip_each_ls
    .tsrc:	
		test	byte [Qallow_ipN.src],1
		jz	.ex		
		setnz	byte[inc_by_port_ip_each_ls.type]
...
inc_by_port_ip_each_ls:
	mov	byte[.table],DB_ID_LS_PORT_EIP
	push 	eax
	call	.inc_by_port_ip
	pop	eax
	mov	byte[.table],DB_ID_LS_PORT_EIP_JOINT
	call	.inc_by_port_ip
	;;;
	ret
.inc_by_port_ip:
	...
	call	strcat_z
	 ...
	 sub ecx,edx
	 mov ebp,ecx
	mov	al,byte[do_ls_by_port_ip_each.protocol]
	stosb
	mov	eax,dword[.addr]
	stosd
	mov	esi,dword[name_esi_tmp]
	...
	mov	esi,main_buffer
	mov	dl,byte[.table]
	mov	ecx,pktypes_inc_l+4+1+4	
	add	ecx,ebp
	add	ecx,dword[cportlp.len]
	call	add_to_db
	;;;
	ret
.addr	dd	0	    
...
on_timer_ls_by_port_ip_each:
	...
	;;;
	ret	
do_lbp_each_timer_ls:
	mov	dword[.nfunc],ecx
	lodsb
	mov	dword[name_esi_tmp_timer],esi
	sub	al,MAXPC_LS_BY_PORT_EIP
	jz	.next
	js	.next
	mov	eax,err_sPC
	call	write
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	   mov	byte[show_ls_by_port_ip_each.table],DB_ID_LS_PORT_EIP
	  call	show_ls_by_port_ip_each
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	   mov	byte[show_ls_by_port_ip_each.table],DB_ID_LS_PORT_EIP_JOINT
	  call	show_ls_by_port_ip_each
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_port_ip_each.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_port_ip_each_ls.table],DB_ID_LS_PORT_EIP
	call	flush_table_ls_port_ip_each_ls
    .ex:
	;;;
	ret
.ferr:
	...
	;;;
	ret
.err2:	db	"ERROR: ls_by_port_ip_each: cannot create file",0xA,0
.ext	dd	0
.nfunc	dd	0
.t_only	db	0
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_port_ip_each_text.table],DB_ID_LS_PORT_EIP
	call	show_ls_by_port_ip_each_text
	mov	edi,S_BUFFER
	mov	esi,t_jt
	call	split
	mov	byte[show_ls_by_port_ip_each_text.table],DB_ID_LS_PORT_EIP_JOINT
	call	show_ls_by_port_ip_each_text
	test	byte[show_ls_by_port_ip_each_text.empty],1
...
show_ls_by_port_ip_each_text:
	...
	jc	near .lpe
	    push ebx
		lodsd
		sub	eax,dword[do_lbp_each_timer_ls.nfunc]
		jnz	.do_nxI
		setnz	byte[.empty]
		mov dword[.tmp_e],esi
		call to_zero
...
flush_table_ls_port_ip_each_ls:
