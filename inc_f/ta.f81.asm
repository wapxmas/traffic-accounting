;;func_ls_by_eport_eip;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_eport_eip_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_EPORT_EIP
	mov	dword[current.nfunc],ecx
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_ls_by_eport_eip
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
...
do_ls_by_eport_eip:
	...
	jnz	.next2
	setz	byte[.first]
	mov	edx,on_timer_ls_by_eport_eip
	call	add_to_timer_f
    .next2:
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_EPORT_EIP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_EPORT_EIP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
	...
	or	bh,bh
	jz	near .ex
	setz	byte[inc_by_eport_eip_ls.type]
	test	byte [QallowPortN.dst],1
	jz	.do_inc_src
	mov	bx,word[QallowPortN.dp]
	call	inc_by_eport_eip_ls
    .do_inc_src:
	test	byte [QallowPortN.src],1
	jz	.ex
	setnz	byte[inc_by_eport_eip_ls.type]
...
.first	db	0
inc_by_eport_eip_ls:
	mov	byte[.table],DB_ID_LS_EPORT_EIP
	 push ebx
	  call	.inc_eport_eip
	 pop ebx
	mov	byte[.table],DB_ID_LS_EPORT_EIP_JOINT
	call	.inc_eport_eip
	;;;
	ret
...
.inc_eport_eip:
	...
	call	strcat_z
	 ...
	 sub ecx,edx
	 mov ebp,ecx
	mov	al,byte[do_ls_by_eport_eip.protocol]
	stosb
...
on_timer_ls_by_eport_eip:
	...
	;;;
	ret	
do_lbep_eip_timer_ls:
	mov	dword[.nfunc],ecx
	lodsb
	mov	dword[name_esi_tmp_timer],esi
	sub	al,MAXPC_LS_BY_EPORT_EIP
	jz	.next
	js	.next
	mov	eax,err_sPC
	call	write
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	   mov	byte[show_ls_by_eport_eip.table],DB_ID_LS_EPORT_EIP
	  call	show_ls_by_eport_eip
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	   mov	byte[show_ls_by_eport_eip.table],DB_ID_LS_EPORT_EIP_JOINT
	  call	show_ls_by_eport_eip
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_eport_eip.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_eport_eip_ls.table],DB_ID_LS_EPORT_EIP
	call	flush_table_ls_eport_eip_ls
    .ex:
	;;;
	ret
.ferr:
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_eport_eip_text.table],DB_ID_LS_EPORT_EIP
	call	show_ls_by_eport_eip_text
	mov	edi,S_BUFFER	
	mov	esi,t_jt
	call	split
	mov	byte[show_ls_by_eport_eip_text.table],DB_ID_LS_EPORT_EIP_JOINT
	call	show_ls_by_eport_eip_text
	test	byte[show_ls_by_eport_eip_text.empty],1
