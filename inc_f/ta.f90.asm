;;ls_by_proto_ip;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_proto_ip_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_PROTO_IP
	mov	dword[current.nfunc],ecx
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_ls_by_proto_ip
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
...
do_ls_by_proto_ip:	
	...
	jnz	.next2
	setz	byte[.first]
	mov	edx,on_timer_ls_by_proto_ip
	call	add_to_timer_f
    .next2:
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_PROTO_IP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_PROTO_IP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
...
.tpsrc	db	0
inc_by_proto_ip_ls:
	mov	byte[.table],DB_ID_LS_PROTO_IP
	call	.inc_by_proto_ip
	mov	byte[.table],DB_ID_LS_PROTO_IP_JOINT
	call	.inc_by_proto_ip
	;;;
	ret
.inc_by_proto_ip:
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	   mov	byte[show_ls_by_proto_ip.table],DB_ID_LS_PROTO_IP
	  call	show_ls_by_proto_ip
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	   mov	byte[show_ls_by_proto_ip.table],DB_ID_LS_PROTO_IP_JOINT
	  call	show_ls_by_proto_ip
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_proto_ip.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_proto_ip_ls.table],DB_ID_LS_PROTO_IP
	call	flush_table_ls_proto_ip_ls
    .ex:
	;;;
	ret
.ferr:
	...
	;;;
	ret
.err2:	db	"ERROR: ls_by_proto_ip: cannot create file",0xA,0
.ext	dd	0
.fnm	dd	0
.nfunc	dd	0
.t_only	db	0
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_proto_ip_text.table],DB_ID_LS_PROTO_IP
	call	show_ls_by_proto_ip_text
	mov	edi,S_BUFFER
	mov	esi,t_jt
	call	split
	mov	byte[show_ls_by_proto_ip_text.table],DB_ID_LS_PROTO_IP_JOINT
	call	show_ls_by_proto_ip_text
	test	byte[show_ls_by_proto_ip_text.empty],1
...
show_ls_by_proto_ip_text:
	...
	jc	near .lpe
	    push ebx
		lodsd
		sub eax,dword[do_lbp_timer_proto_ls.nfunc]
		jnz .do_nxI
		setnz byte[.empty]
		mov dword[.tmp_e],esi
		call to_zero
