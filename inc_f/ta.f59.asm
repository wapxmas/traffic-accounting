;;ls_by_each_ip;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_each_ip_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_EACH_IP
	mov	dword[current.nfunc],ecx
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_ls_by_each_ip
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
...
do_ls_by_each_ip:
	...
	jnz	.nx
	setz	byte[.first]
	mov	edx,on_timer_ls_by_each_ip
	call	add_to_timer_f
    .nx:
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_EACH_IP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_EACH_IP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
	...
	jz	.tsrc
	setz	byte[inc_by_ip_ls_table.type]
	mov	eax,dword[packet.da]
	bswap	eax
	call	inc_by_ip_ls_table
    .tsrc:	
		test	byte [Qallow_ipN.src],1
		jz	.ex
		setnz	byte[inc_by_ip_ls_table.type]
...
.first	db	0	
inc_by_ip_ls_table:
	mov	byte[.table],DB_ID_LBE_LS_EACH
	push	eax
	 call	.inc_by_ip
	pop	eax
	mov	byte[.table],DB_ID_LBE_LS_EACH_JOINT
	call	.inc_by_ip
	;;;
	ret
;;INPUT: eax=IPnum;
...
on_timer_ls_by_each_ip:
	...
	;;;
	ret
do_ls_by_each_ip_timer:
	mov	dword[.nfunc],ecx
	lodsb
	mov	dword[name_esi_tmp_timer],esi
	sub	al,MAXPC_LS_BY_EACH_IP
	jz	.next
	js	.next
	mov	eax,err_sPC
	call	write
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	  mov	byte[show_ls_by_each_ip.table],DB_ID_LBE_LS_EACH
	  call	show_ls_by_each_ip
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	  mov	byte[show_ls_by_each_ip.table],DB_ID_LBE_LS_EACH_JOINT
	  call	show_ls_by_each_ip
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_each_ip.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_each_ip.table],DB_ID_LBE_LS_EACH
	call	flush_table_ls_each_ip
    .ex:
	;;;
	ret
.ferr:
	...
	;;;
	ret
.err2:	db	"ERROR: ls_by_each_ip: cannot create file",0xA,0
.ext	dd	0
.nfunc	dd	0
.t_only	db	0
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_each_ip_text.table],DB_ID_LBE_LS_EACH
	call	show_ls_by_each_ip_text
	mov	edi,S_BUFFER
	mov	esi,t_jt
	call	split	
	mov	byte[show_ls_by_each_ip_text.table],DB_ID_LBE_LS_EACH_JOINT
	call	show_ls_by_each_ip_text
	test	byte[show_ls_by_each_ip_text.empty],1
...
show_ls_by_each_ip_text:
	...
	jc	.lpe
	    push ebx
		lodsd
		sub eax,dword[do_ls_by_each_ip_timer.nfunc]
		jnz .do_nxI
		setnz byte[.empty]
		mov dword[.tmp_e],esi
		call to_zero
