;;ls_by_ip;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_ip_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_IP
...
do_ls_by_ip:
	...
	jnz	.nx
	setz	byte[.first]
	mov	edx,on_timer_ls_by_ip
	call	add_to_timer_f
    .nx:
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_IP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_IP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
	...
	or	bh,bh
	jz	.ex
	call	inc_by_ip_ls_table_group
    .ex:	
	;;;
	ret
.first	db	0	
inc_by_ip_ls_table_group:
	mov	byte[.table],DB_ID_LBI
	call	.inc_by_ip
	mov	byte[.table],DB_ID_LBI_JOINT
	call	.inc_by_ip
	;;;
	ret
;;INPUT: eax=IPnum;
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	  mov	byte[show_ls_by_ip.table],DB_ID_LBI
	  call	show_ls_by_ip
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	  mov	byte[show_ls_by_ip.table],DB_ID_LBI_JOINT
	  call	show_ls_by_ip
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_ip.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_ip.table],DB_ID_LBI
	call	flush_table_ls_ip
    .ex:
	;;;
	ret
.ferr:
	...
	;;;
	ret
.err2:	db	"ERROR: ls_by_ip: cannot create file",0xA,0
.ext	dd	0
.nfunc	dd	0
.t_only	db	0
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_ip_text.table],DB_ID_LBI
	call	show_ls_by_ip_text
	mov	edi,S_BUFFER
	mov	esi,t_jt
	call	split	
	mov	byte[show_ls_by_ip_text.table],DB_ID_LBI_JOINT
	call	show_ls_by_ip_text
	test	byte[show_ls_by_ip_text.empty],1
...
show_ls_by_ip_text:
	...
	jc	.lpe
	    push ebx
		lodsd
		sub eax,dword[do_ls_by_ip_timer.nfunc]
		jnz .do_nxI
		setnz byte[.empty]
		mov 	dword[.tmp_e],esi
		call	to_zero
