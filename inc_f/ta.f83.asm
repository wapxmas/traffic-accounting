;;func_ls_by_eproto_eip;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_eproto_eip_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_EPROTO_EIP
	mov	dword[current.nfunc],ecx	
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_ls_by_eproto_eip
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
...
do_ls_by_eproto_eip:
	...
	jnz	.next2
	setz	byte[.first]
	mov	edx,on_timer_ls_by_eproto_eip
	call	add_to_timer_f	
    .next2:	
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_EPROTO_EIP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_EPROTO_EIP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
	...
	jz	.tsrc
	setz	byte[inc_by_eproto_eip_ls.type]
	mov	eax,dword[packet.da]
	bswap	eax
	call	inc_by_eproto_eip_ls
    .tsrc:	
		test	byte [Qallow_ipN.src],1
		jz	.ex		
		setnz	byte[inc_by_eproto_eip_ls.type]
...
.st	db	0
.tpsrc	db	0
inc_by_eproto_eip_ls:    
    mov	byte[.table],DB_ID_LS_EPROTO_EIP
    push eax
     call .inc_by_eproto_eip  
    pop eax
    mov	byte[.table],DB_ID_LS_EPROTO_EIP_JOINT
    call .inc_by_eproto_eip
    ;;;
    ret
.inc_by_eproto_eip:
...
on_timer_ls_by_eproto_eip:
	...
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_lbp_each_timer_eproto_ls
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
	...
	;;;
	ret	
do_lbp_each_timer_eproto_ls:
	mov	dword[.nfunc],ecx
	lodsb
	mov	dword[name_esi_tmp_timer],esi
	sub	al,MAXPC_LS_BY_EPROTO_EIP
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit		
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	   mov	byte[show_ls_by_eproto_eip.table],DB_ID_LS_EPROTO_EIP
	  call	show_ls_by_eproto_eip
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	   mov	byte[show_ls_by_eproto_eip.table],DB_ID_LS_EPROTO_EIP_JOINT
	  call	show_ls_by_eproto_eip
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_eproto_eip.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_eproto_eip_ls.table],DB_ID_LS_EPROTO_EIP
	call	flush_table_ls_eproto_eip_ls
    .ex:
	;;;
	ret
.ferr:
	...
	;;;
	ret
.err2:	db	"ERROR: ls_by_eproto_eip: cannot create file",0xA,0
.ext	dd	0
.fnm	dd	0
.nfunc	dd	0
.t_only	db	0
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_eproto_eip_text.table],DB_ID_LS_EPROTO_EIP
	call	show_ls_by_eproto_eip_text
	mov	edi,S_BUFFER
	mov	esi,t_jt
	call	split
	mov	byte[show_ls_by_eproto_eip_text.table],DB_ID_LS_EPROTO_EIP_JOINT
	call	show_ls_by_eproto_eip_text
	test	byte[show_ls_by_eproto_eip_text.empty],1
...
show_ls_by_eproto_eip_text:
	...
	jc	near .lpe
	    push ebx
		lodsd
		sub eax,dword[do_lbp_each_timer_eproto_ls.nfunc]
		jnz .do_nxI
		setnz byte[.empty]	    	
		mov dword[.tmp_e],esi
		call to_zero
