;;ls_by_eproto_ip;
	sub	ecx,ecx
    .lp:
	mov	esi,ls_by_eproto_ip_buffer
	lodsd
	dec	eax
	imul	ebx,ecx,MAXL_LS_BY_EPROTO_IP
	mov	dword[current.nfunc],ecx	
	add	esi,ebx
	    push ecx
		push	eax
	    	    call do_ls_by_eproto_ip
		pop	eax
	    pop ecx
	inc	ecx
	cmp	ecx,eax
...
do_ls_by_eproto_ip:
	...
	jnz	.next2
	setz	byte[.first]
	mov	edx,on_timer_ls_by_eproto_ip
	call	add_to_timer_f	
    .next2:	
	lodsb
	mov	dword[name_esi_tmp],esi
	cmp	al,MAXPC_LS_BY_EPROTO_IP
	jz	.nex_1
	sub	al,(MAXPC_LS_BY_EPROTO_IP-1)
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit
...
.tpsrc	db	0
inc_by_eproto_ip_ls:
    mov	byte[.table],DB_ID_LS_EPROTO_IP
    call .inc_by_eproto_ip
    mov	byte[.table],DB_ID_LS_EPROTO_IP_JOINT
    call .inc_by_eproto_ip
    ;;;
    ret
.inc_by_eproto_ip:
...
on_timer_ls_by_eproto_ip:
	...
	;;;
	ret	
do_lbp_timer_eproto_ls:
	mov	dword[.nfunc],ecx
	lodsb
	mov	dword[name_esi_tmp_timer],esi
	sub	al,MAXPC_LS_BY_EPROTO_IP
	jz	.next
	mov	eax,err_sPC
	call	write
	call	@exit		
...
.time_allowed:
	...
	call	out_head
	 ...
	 call	out_cr_traf
	  call	out_table
	   mov	byte[show_ls_by_eproto_ip.table],DB_ID_LS_EPROTO_IP
	  call	show_ls_by_eproto_ip
	 call	out_table_end	 
	 ;;joint;
	 call	out_jt_traf
	  call	out_table
	   mov	byte[show_ls_by_eproto_ip.table],DB_ID_LS_EPROTO_IP_JOINT
	  call	show_ls_by_eproto_ip
	 call	out_table_end
	call	out_end
	test	byte[show_ls_by_eproto_ip.empty],1
...
.ex_flush:
	mov	eax,6
	int	0x80
	mov	byte[flush_table_ls_eproto_ip_ls.table],DB_ID_LS_EPROTO_IP
	call	flush_table_ls_eproto_ip_ls
    .ex:
	;;;
	ret
.ferr:
	...
	;;;
	ret
.err2:	db	"ERROR: ls_by_eproto_ip: cannot create file",0xA,0
.ext	dd	0
.fnm	dd	0
.nfunc	dd	0
.t_only	db	0
...
.text_only:
	...
	mov	esi,t_cr
	call	split
	mov	byte[show_ls_by_eproto_ip_text.table],DB_ID_LS_EPROTO_IP
	call	show_ls_by_eproto_ip_text
	mov	edi,S_BUFFER
	mov	esi,t_jt
	call	split
	mov	byte[show_ls_by_eproto_ip_text.table],DB_ID_LS_EPROTO_IP_JOINT
	call	show_ls_by_eproto_ip_text
	test	byte[show_ls_by_eproto_ip_text.empty],1
...
show_ls_by_eproto_ip_text:
	...
	jc	near .lpe
	    push ebx
		lodsd
		sub eax,dword[do_lbp_timer_eproto_ls.nfunc]
		jnz .do_nxI
		setnz byte[.empty]	    	
		mov dword[.tmp_e],esi
		call to_zero
